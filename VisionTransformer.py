...
class SelfAttention(nn.Module):
    def __init__(self, embedding_dims, num_heads, dropout):
        ...
        self.fc = nn.Linear(self.head_dims * self.num_heads, self.embedding_dims)
    ...
    def forward(self, query, key, value, mask):
        ...
        query = query.reshape(batch, query_len, self.num_heads, self.head_dims)
        key = key.reshape(batch, key_len, self.num_heads, self.head_dims)
        value = value.reshape(batch, value_len, self.num_heads, self.head_dims)
        ...
        out = torch.einsum('bhqv,bvhd->bqhd', [attention_score, value]).reshape(
            batch, query_len, self.num_heads*self.head_dims
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, embedding_dims, num_heads, dropout, forward_expansion, layer_norm_eps):
        ...
        self.attention = SelfAttention(embedding_dims, num_heads, dropout)
...
class ViT(nn.Module):
    def __init__(self, patch_height, patch_width, max_len, embedding_dims, num_heads, forward_expansion, num_layers, dropout, layer_norm_eps, num_classes
    ):
        super(ViT, self).__init__()
        
        self.vit_blocks = nn.Sequential(
            *[TransformerBlock(embedding_dims, num_heads, dropout, forward_expansion, layer_norm_eps)
    ...
    def forward(self, images):        
        ...
        patches = patches.permute(0, 2, 3, 1, 4, 5) # (0, 2, 3, 1, 4, 5)
...
if __name__ == "__main__":
    ...
    model = ViT(
        ...
        num_layers = 4,
        forward_expansion = 4,
        max_len = int((32*32)/(16*16)), # ((32*32)/(16*16))
        layer_norm_eps = 1e-5,
        num_classes = 10,
    )

    a = torch.randn(32, 3, 32, 32) # (32, 3, 32, 32)
    output = model(a)
    # print(output.shape)